...
ObjectLabel *ObjectParse::
LabelFromKey(int key) const
{
  // Find label by name
  for (int i = 0; i < NLabels(); i++) {
    ObjectLabel *label = Label(i);
    if (label->AssignmentKeystroke() == key) return label;
  }
  return NULL;
}
...
ObjectModel *ObjectParse::
Model(const char *name) const
{
  // Find model by name
...
ReadAsciiFile(const char *filename)
{
  ...
  while (fgets(buffer, 4096, fp)) {
    ...
    // Parse line
    char keyword[4096], name[4096], fname[4096], key[4096];
    double cx, cy, cz, r, h, score, red, green, blue, m[16];
    ...
    if (sscanf(bufferp, "%s", keyword) == (unsigned int) 1) {
      if (!strcmp(keyword, "A")) {
        ...
        if (sscanf(bufferp, "%s%d%lf%lf%lf%lf%lf%d%s%d%d%d%d%d", keyword, 
          &detection_index, &cx, &cy, &cz, &r, &h, &dummy, fname, &dummy, &dummy, &dummy, &dummy, &dummy) != (unsigned int) 14) {
        ...
        const char *pointset_filename = (strcmp(fname, "None")) ? fname : NULL;
        ...
        if (sscanf(bufferp, "%s%d%lf%lf%lf%lf%lf%s%s%d%d%d%d%d", keyword, 
          &label_index, &cx, &cy, &cz, &r, &h, name, fname, &dummy, &dummy, &dummy, &dummy, &dummy) != (unsigned int) 14) {
          fprintf(stderr, "Error parsing model at line %d of %s\n", line_number, filename);
          return 0;
        }

        // Create model
        ObjectModel *model = (strcmp(name, "None") && strcmp(name, "0")) ? Model(name) : NULL;
        if (!model) {
          R3Point origin(cx, cy, cz);
          ObjectLabel *label = read_labels.Kth(label_index);
          const char *model_name = (strcmp(name, "None") && strcmp(name, "0")) ? name : NULL;
          const char *pointset_filename = (strcmp(fname, "None") && strcmp(name, "0")) ? fname : NULL;
          model = new ObjectModel(pointset_filename, origin, r, h, model_name);
        ...
        // Parse label
        if (sscanf(bufferp, "%s%s%lf%lf%lf%lf%lf%d%s%lf%lf%lf", keyword, 
          name, &cx, &cy, &cz, &r, &h, &dummy, key, &red, &green, &blue) != (unsigned int) 12) {
          fprintf(stderr, "Error parsing label at line %d of %s\n", line_number, filename);
          return 0;
        }

        // Create label
        ObjectLabel *label = (strcmp(name, "None") && strcmp(name, "0")) ? Label(name) : NULL; 
        if (!label) {
          R3Point origin(cx, cy, cz);
          RNRgb color(red, green, blue);
          int assignment_keystroke = key[0];
          label = new ObjectLabel(name, origin, r, h, assignment_keystroke, color);
          InsertLabel(label);
        }
...
WriteAsciiFile(const char *filename) const
{
  ...
  for (int i = 0; i < NLabels(); i++) {
    ...
    unsigned char key = (char) label->AssignmentKeystroke();
    fprintf(fp, "L %s  %g %g %g  %g %g  %d  %c  %g %g %g\n", name, 
  ...
  for (int i = 0; i < NModels(); i++) {
    ...
    fprintf(fp, "M  %d  %g %g %g  %g %g   %s %s  %d %d  0 0 0\n", 
      label_index, 
      origin.X(), origin.Y(), origin.Z(), 
