...
// special to gsvshow
static R2Grid ***detections = NULL;
static R2Grid ***probabilities = NULL;
...
ReadUndistortedImage(GSVImage *image)
{
	...
	sprintf(image_name, "%s/%s/%02d_%06d_%02d_UndistortedImage.jpg",
...
DrawScans(int side, RNBoolean transform, int color_scheme)
{
	...
	if (color_toggle) {
		...
		for (int ir = 0; ir < scene->NRuns(); ir++) {
			GSVRun *run = scene->Run(ir);
			for (int is = 0; is < run->NSegments(); ++is, ++ns) {
				GSVSegment *segment = run->Segment(is);
				LoadSegmentColor(segment, color_scheme);
				for (int ia = 0; ia < segment->NScans(); ia++) {
					if (ia != 1) {
						...
						// read pfm file
						R2Grid *probability = probabilities[ns][ia];
						R2Grid *detection = detections[ns][ia];
						for (int ie = 0; ie < scan->NScanlines(); ie++) {
							...
							for (int ik = 0; ik < scanline->NPoints(); ik++) {
								...
								double prediction = probability->GridValue(ie, ik);
								//// HARDCODE traffic light
								if ((int)detection->GridValue(ie, ik) != 34) RNLoadRgb(RNRgb(1.0, 0.0, 0.0));
...
void GLUTMouse(int button, int state, int x, int y)
{
	...
	RNBoolean drag = (fabs((double) x - last_mouse_down[0]) > 10) | (fabs((double) y - last_mouse_down[1]) > 10);
...
ParseArgs(int argc, char **argv)
{
	...
	while (argc > 0) {
		if ((*argv)[0] == '-') {
			...
			else if (!strcmp(*argv, "-create_correspondences")) {
...
int main(int argc, char **argv)
{
	...
	// read in pfm grids
	int nsegments = scene->NSegments();
	probabilities = new R2Grid **[nsegments];
	detections = new R2Grid **[nsegments];
	for (int i = 0; i < nsegments; i++) {
		// two scans per segment, ignore middle
		probabilities[i] = new R2Grid *[3];
		detections[i] = new R2Grid *[3];
	}

	// go through each run and read in pfms
	for (int ir = 0; ir < scene->NRuns(); ir++) {
		GSVRun *run = scene->Run(ir);
		int ns = 0;
		for (int is = 0; is < run->NSegments(); ++is, ++ns) {
			...
			for (int ia = 0; ia < segment->NScans(); ia++) {
				if (ia != 1) {
					char pfmFilename[4096];
					sprintf(pfmFilename, "gsv_data/laser_images/%s/%02d_%02d_SA_Detection.grd", run->Name(), is, ia);
					R2Grid *detection = new R2Grid();
					detection->Read(pfmFilename);

					sprintf(pfmFilename, "gsv_data/laser_images/%s/%02d_%02d_SA_Probabilities.grd", run->Name(), is, ia);
					R2Grid *probability = new R2Grid();
					probability->Read(pfmFilename);

					sprintf(pfmFilename, "gsv_data/laser_images/%s/%02d_%02d_SA_PointIndex.grd", run->Name(), is, ia);
					R2Grid *point = new R2Grid();
					point->Read(pfmFilename);

					sprintf(pfmFilename, "gsv_data/laser_images/%s/%02d_%02d_SA_Scanline.grd", run->Name(), is, ia);
					R2Grid *scan = new R2Grid();
					scan->Read(pfmFilename);

					detections[ns][ia] = new R2Grid(scan->Maximum(), point->Maximum());
					probabilities[ns][ia] = new R2Grid(scan->Maximum(), point->Maximum());
					for (int i = 0; i < detections[ns][ia]->XResolution(); i++) {
						for (int j = 0; j < detections[ns][ia]->YResolution(); j++) {
							probabilities[ns][ia]->SetGridValue(i, j, R2_GRID_UNKNOWN_VALUE);
							detections[ns][ia]->SetGridValue(i, j, R2_GRID_UNKNOWN_VALUE);
						}
					}
					for (int i = 0; i < probability->XResolution(); i++) {
						for (int j = 0; j < probability->YResolution(); j++) {
							if (scan->GridValue(i, j) != R2_GRID_UNKNOWN_VALUE && point->GridValue(i, j) != R2_GRID_UNKNOWN_VALUE) {
								probabilities[ns][ia]->SetGridValue((int)scan->GridValue(i, j), (int)point->GridValue(i, j), probability->GridValue(i, j));
								detections[ns][ia]->SetGridValue((int)scan->GridValue(i, j), (int)point->GridValue(i, j), detection->GridValue(i, j));
							}
						}
					}
				}
