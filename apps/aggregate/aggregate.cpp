...
public:
	RCNNBoundingBox(string object, R2Box bbox, double score);
	string Object(void);
	R2Box BBox(void);
	double Score(void);
...
RCNNBoundingBox::RCNNBoundingBox(string object, R2Box bbox, double score) :
object(object),
bbox(bbox),
score(score)
{
...
static vector<string> splitString(string line, char delim = ',') {
	...
	if (string_start != length)	arguments.push_back(line.substr(string_start));
...
static GSVScene *ReadScene(const char *filename) {
	...
	if (!scene->ReadFile(filename)) {
		fprintf(stderr, "Unable to read scene: %s\n", filename);
		delete scene;
		return NULL;
	}
			...
			for (int ip = 0, pn = 0; ip < segment->NPanoramas(); ++ip, ++pn) {
				...
				for (int ii = 0; ii < panorama->NImages(); ++ii, ++in) {
					...
					while (getline(object_file, line)) {
						...
						RCNNBoundingBox *bbox = new RCNNBoundingBox(object, R2Box(xmin, ymin, xmax, ymax), score);
...
//////////////////////////////////////////////////////////////////////////////
// Aggregate RCNN votes
//////////////////////////////////////////////////////////////////////////////

static int AggregateRCNNPairs(void) {
	// Start statistics
	RNTime start_time;
	start_time.Read();

	double xstart = scene->BBox().XMin();
	double ystart = scene->BBox().YMin();
	double zstart = scene->BBox().ZMin();

	double xlength = scene->BBox().XLength();
	double ylength = scene->BBox().YLength();
	double zlength = scene->BBox().ZLength();

	double increment = 2.0;
	double xres = xlength * increment;
	double yres = ylength * increment;
	double zres = zlength * increment;

	long int i = 0.0;
	for (int ii = 0; ii < scene->NImages(); ++ii) {
		GSVImage *image = images[ii];
		for (int ix = 0; ix < xres; ++ix) {
			for (int iy = 0; iy < yres; ++iy) {
				for (int iz = 0; iz < zres; ++iz) {
					double x = xstart + (ix / (double)xres) * xlength;
					double y = ystart + (iy / (double)yres) * ylength;
					double z = zstart + (iz / (double)zres) * zlength;

					R3Point point = R3Point(x, y, z);
					//R2Point p = image->UndistortedPosition(point);

					int nobjects = object_map.size();
					for (int io = 0; io < nobjects; ++io, ++i) {
						if (i % 10000 == 0) printf("%0.4f%%\n", 100 * (double)i / (double)(scene->NImages() * xres * yres * zres * nobjects));
					}
				}
			}
		}
	...
	// Print statistics
	if (print_verbose) {
		printf("Creating aggregate data ...\n");
		printf("  Time = %.2f seconds\n", start_time.Elapsed());
		fflush(stdout);
	}
...
//////////////////////////////////////////////////////////////////////////////
// Parsing argument functions and Main
//////////////////////////////////////////////////////////////////////////////

// how should this program be used
static int Usage(void) {
	fprintf(stderr, "Usage: aggregate.cpp [-v] input_scene_name [-options ...]\n");
	return 0;
}
