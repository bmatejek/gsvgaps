...
static R3Grid *truth2grd(int xLength, int yLength, int zLength, R3Box boundingBox, string object) {
  ...
  char *truthFilename = "truth_data.txt";
...
static int GenerateGrids(void) {
  ...
  for (unsigned int io = 0; io < objects.size(); io++) {
    ...
    string descriptionFile = objectName + string("/description.txt");
    ifstream description(descriptionFile.c_str());
	...
	for (int ip = 0; ip < segment->NPanoramas(); ip++) {
	  ...
	  for (int ii = 0; ii < panorama->NImages() - 1; ii++) {
	    ...
	    snprintf(vocFilename, 127, "%s/%02d/%02d_%06d_%02d_UndistortedImage.txt", objectName.c_str(), run_number, is, ip, ii);
		...
		double xmin = atof(vocBox[0].c_str());
		double ymin = atof(vocBox[1].c_str());
		double xmax = atof(vocBox[2].c_str());
		double ymax = atof(vocBox[3].c_str());
		double score = atof(vocBox[5].c_str());
		
		// only consider boxes above a certain threshold
		if (score > score_cutoff) {
		  // convert from VOC coordinate to GSV coordinate
		  xmin = VOCToGSV(xmin, true, image->Height(), image->Width());
		  ymin = VOCToGSV(ymin, false, image->Height(), image->Width());
		  xmax = VOCToGSV(xmax, true, image->Height(), image->Width());
		  ymax = VOCToGSV(ymax, false, image->Height(), image->Width());
		  
		  // switch ymin and ymax to account for GSV/VOC differences
		  double tmp = ymin;
		  ymin = ymax;
		  ymax = tmp;
		  if (score_cutoff < 0.0)
		    score -= score_cutoff;
		  // for every pixel in the box, find accompanying ray
		  for (int x = floor(xmin); x <= ceil(xmax); x++) {
		    for (int y = floor(ymin); y <= ceil(ymax); y++) {
		      // use a reduced span if guessing the distance to the object
		      if (guess_distance) {
			// find the estimated distance to the object
			double image_plane_distance = EstimatedImagePlaneDistance(ymin, ymax, objectHeight, image);
			...
			objGrid->RasterizeWorldSpan(start, end, score / ((ymax - ymin) * (xmax - xmin)));
		      } 
		      // otherwise the span goes from near the camera to the edge of the bounding box
		      else {
			R3Ray ray = image->RayThroughUndistortedPosition(R2Point((double) x, (double) y));
			R3Point start = ray.Point(1.2);
			R3Point end;
			R3Intersects(ray, boundingBox, &end);
			objGrid->RasterizeWorldSpan(start, end, score);
		      }
		    }
		  }
		}
    ...
    *mergeGrid *= *objGrid;
    ...
    if (ignore_cutoff)
      mergeFilename = string(output_grid_directory) + string("no_cutoff_merged_") + objectName + string(".grd");
    else
      mergeFilename = string(output_grid_directory) + string("merged_") + objectName + string(".grd");
    ...
    // write object grid
    string objectFilename = string(output_grid_directory) + objectName + string(".grd");
