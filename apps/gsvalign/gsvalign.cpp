...
static int
WriteFeatureGrids(GSVPoseOptimization *optimization, const char *output_directory = NULL,  const char *parameterization = "DA")
{
  // Start statistics
  RNTime start_time;
  start_time.Read();
    ...
    for (int is = 0; is < run->NSegments(); is++) {
      ...
      for (int ia = 0; ia < segment->NScans(); ia++) {
        ...
        for (int k = 0; k < optimization->NFeatures(); k++) {
          ...
          RNScalar x = scanline->ScanIndex();
          RNScalar y = feature->scan_point_index;
          if (parameterization[0] == 'D') {
            x = scanline->TravelDistance();
          }
          if (parameterization[1] == 'A') {
            ...
            RNLength height = feature->scan_position.Z() - scanline->EstimatedGroundZ();
            y = height;
          }

          printf("%g %g\n", x, y);

          // Rasterize point at feature position
          R2Point p = feature_grid.GridPosition(x,y);
          int ix = (int) (p.X() + 0.5);
          int iy = (int) (p.Y() + 0.5);
          if ((ix < 0) || (ix >= feature_grid.XResolution())) continue;
          if ((iy < 0) || (iy >= feature_grid.XResolution())) continue;
          feature_grid.SetGridValue(ix, iy, feature->feature_type);
        }
        
        // Write feature grid
        sprintf(image_name, "%s/%s/%02d_%02d_%s_ScanFeatures.grd", output_directory, run->Name(), is, ia, parameterization);
...
void AtExit(void)
{
  ...
  // Write feature grids
  if (output_feature_grid_directory) {
    if (!WriteFeatureGrids(optimization, output_feature_grid_directory)) exit(-1);
  }
...
ParseArgs(int argc, char **argv)
{
  ...
  while (argc > 0) {
    if ((*argv)[0] == '-') {
      ...
      else if (!strcmp(*argv, "-output_feature_grids")) { argc--; argv++; output_feature_grid_directory = *argv; }
  ...
  if (!output_scene_name && !output_correspondences_name && !output_transformations_name && !output_inertias_name && 
      !output_feature_grid_directory && !output_world_distance_error_plot_name && !output_image_distance_error_plot_name) {
    interactive = TRUE;
  }
