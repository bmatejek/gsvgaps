...
static int selected_grid_index = 0;
...
void GLUTRedraw(void)
{
    ...
    for (int i = 0; i < grids->NGrids(); i++) {
        if ((selected_grid_index >= 0) && (i != selected_grid_index)) continue;
        DrawGrid(grids->Grid(i));
    }
...
void GLUTMotion(int x, int y)
{
    ...
    R3Grid *grid = grids->Grid(selected_grid_index);
...
static R3Viewer *
CreateViewer(R3GridArray *grids)
{
    // Start statistics
    RNTime start_time;
    start_time.Read();
