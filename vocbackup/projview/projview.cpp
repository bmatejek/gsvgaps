...
void GLUTRedraw(void) {
	...
	string proj_file = "proj_truth/" + string(object) + "/" + fileparts[1] + "/" + filename[0] + ".txt";
	ifstream projections;
	projections.open(proj_file.c_str());
	if (!projections.is_open()) {
		...
		string line;
		while (getline(projections, line)) {
...
void GLUTInit(int *argc, char **argv) {
	...
	GLUTwindow = glutCreateWindow("projview");
...
static int Usage(void) {
	fprintf(stderr, "projview [object] [first_image]\n");
	return 0;
}
