...
static int DetectObjects(void) {
	double total = (double)scene->NScanlines();
	int c = 0;
	printf("%f\n", total);
	printf("%d\n", scene->NRuns());
	for (int r = 0; r < scene->NRuns(); r++) {
		...
		for (int s = 0; s < run->NSegments(); s++) {
			...
			for (int a = 0; a < segment->NScans(); a++) {
				...
				if (!scan->ReadPoints()) { fprintf(stderr, "Unable to read points\n"); return 0; }
				// read all of the points
				for (int l = 0; l < scan->NScanlines(); ++l, ++c) {
					GSVScanline *scanline = scan->Scanline(l);
					for (int p = 0; p < scanline->NPoints(); ++p) {
						printf("%d\n", c);
						//printf("%0.4f%%\n", 100 * (double)c / total);

						// get the point in the world
						R3Point world = scanline->PointPosition(p);
	...
	if (false) {
		...
		int xresolution = output->XResolution();
		int yresolution = output->YResolution();
		int zresolution = output->ZResolution();
		...
		for (int x = 0; x < xresolution; ++x) {
			for (int y = 0; y < yresolution; ++y) {
				for (int z = 0; z < zresolution; ++z, ++i) {
					...
					printf("%0.5f%%\n", 100 * i / (double)(xresolution * yresolution * zresolution));
					...
					// update this grid value
					output->SetGridValue(x, y, z, add_probability);
				}
			}
		}
	}
