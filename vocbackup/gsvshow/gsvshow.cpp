				...
				if (ia != 1) {
					...
					for (int ie = 0; ie < scan->NScanlines(); ie++) {
						...
						for (int ik = 0; ik < scanline->NPoints(); ik++) {
							...
							//if (ie == 2491 && ik == 94) { printf("HEre\n"); }
							// get prediction of this point
							double prediction = pfm->GridValue(ie, ik);
							if (prediction > 0.1) {
								//printf("%d %d\n", pfm->XResolution(), pfm->YResolution());
...
int main(int argc, char **argv)
{
		...
		for (int is = 0; is < run->NSegments(); ++is, ++ns) {
			...
			for (int ia = 0; ia < segment->NScans(); ia++) {
				if (ia != 1) {
					...
					sprintf(pfmFilename, "gsv_data/laser_images/%s/%02d_%02d_SA_stop_sign_Mean.grd", run->Name(), is, ia);
					R2Grid *prediction = new R2Grid();
					prediction->Read(pfmFilename);
					...
					pfms[ns][ia] = new R2Grid(scan->Maximum(), point->Maximum());
					for (int i = 0; i < prediction->XResolution(); i++) {
						for (int j = 0; j < prediction->YResolution(); j++) {
							if (prediction->GridValue(i, j) > 0.05) {
								pfms[ns][ia]->SetGridValue(scan->GridValue(i, j), point->GridValue(i, j), prediction->GridValue(i, j));
							}
						}
					}
				}
	...
	if (update_pixel_features) {
		...
		if (!strcmp(input_scene_name, "test.gsv")) cull_box = R3Box(-RN_INFINITY, -60, -RN_INFINITY, -500, RN_INFINITY, RN_INFINITY);
